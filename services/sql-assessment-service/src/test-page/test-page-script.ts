...
const successRateTasks: Record<string, Question> = {
    "t0": {
...
const feedbackTasks: Record<string, Question> = {
    "t0": {
...
const plausibilityTasks: Record<string, Question> = {
    "t0": {
...
const testSetups: Record<string, TestSetup> = {
    "Test1": {
        id: "Test1",
        type: "text",
        feedback: false,
...
let testSetupId: string | null = null;
let testSetup: TestSetup | null = null;
let currentQuestionIndex = 0;
let report: Record<string, { grade?: number, answer: string[], startTime: number, submitTime: number, submitAttempts: number, feedback: string }> = {};
let startTime = 0;
...
const questionImage = document.getElementById("question-image")! as HTMLImageElement;
const inputContainer = document.getElementById("input-container")!;
const submitBtn = document.getElementById("submit-btn")! as HTMLButtonElement;
const nextBtn = document.getElementById("next-btn")! as HTMLButtonElement;
const startBtn = document.getElementById("start-btn")! as HTMLButtonElement;
const testDropdown = document.getElementById("test-dropdown")! as HTMLSelectElement;
...
function loadQuestion() {
    ...
    if (question.isIntroduction) {
        ...
        testSetup.options?.forEach(option => {
...
async function submitAnswer(showFeedback: boolean) {
    if (!testSetup) return;
    const question = testSetup.tasks[Object.keys(testSetup.tasks)[currentQuestionIndex]];;
    let userAnswer = "";

    if (testSetup.type === "text") {
        userAnswer = (document.getElementById("user-input") as HTMLInputElement).value;
    } else {
        const selected = document.querySelector<HTMLInputElement>("input[name='user-input']:checked");
