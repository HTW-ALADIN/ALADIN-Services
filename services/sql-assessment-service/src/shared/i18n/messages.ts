...
export const MESSAGES: MessageCatalogue = {
    en: {
        ...
        FEEDBACK_QUERY_EXECUTION_ERROR:   'Query is not executable due to following syntax error:',
        FEEDBACK_QUERY_COMPARISON_ERROR:  'Unable to execute query comparison: {{value}}',
        // grading feedback — AST parsing
        ...
        FEEDBACK_SQL_CLAUSE_TYPE:         'Incorrect SQL clause, the task requires a clause of type: {{value}}',
        // grading feedback — columns
        ...
        FEEDBACK_COLUMNS_SOLUTION:        'The task requires the selection of the following columns: {{value}}',
        // grading feedback — LIMIT / OFFSET
        ...
        // grading feedback — JOIN
        FEEDBACK_JOIN_COUNT_WRONG:        'Incorrect Join statement: Query does not include the correct number of Joins.',
        FEEDBACK_JOIN_WRONG_TABLE:        'Incorrect Join statement: Query uses incorrect table in Join statement.',
        FEEDBACK_JOIN_WRONG_TYPE:         'Incorrect Join statement: Query uses wrong Join type.',
        FEEDBACK_JOIN_WRONG_CONDITION:    'Incorrect Join statement: Query uses incorrect Join condition.',
        ...
        FEEDBACK_DISTINCT_EXTRA:          'DISTINCT keyword should not be used in this query.',
        FEEDBACK_DISTINCT_EXTRA_SOLUTION: 'The query should use plain SELECT without DISTINCT.',
        FEEDBACK_DISTINCT_STRATEGY:       'Note: different DISTINCT implementation strategy (student: {{student}}, reference: {{reference}}).',
        FEEDBACK_CTE_INCORRECT:           'Incorrect CTE usage.',
        FEEDBACK_CTE_SOLUTION:            'Expected CTEs: [{{expected}}], got: [{{received}}].',
        ...
        FEEDBACK_WINDOW_MISSING_SOLUTION: 'The query should use a window function with OVER.',
        FEEDBACK_WINDOW_EXTRA:            'Window function (OVER) should not be used in this query.',
        FEEDBACK_WINDOW_PARTITION:        'Incorrect PARTITION BY in window function.',
        FEEDBACK_WINDOW_PARTITION_SOLUTION: 'Expected PARTITION BY: [{{expected}}], got: [{{received}}].',
        FEEDBACK_WINDOW_ORDER_BY:         'Incorrect ORDER BY in window function.',
        FEEDBACK_WINDOW_ORDER_BY_SOLUTION: 'Expected window ORDER BY: [{{expected}}], got: [{{received}}].',
        FEEDBACK_SUBQUERY_COUNT:                   'Incorrect number of subqueries: expected {{expected}}, got {{received}}.',
        FEEDBACK_SUBQUERY_COUNT_SOLUTION_SINGULAR: 'The query should contain exactly {{count}} subquery.',
        FEEDBACK_SUBQUERY_COUNT_SOLUTION_PLURAL:   'The query should contain exactly {{count}} subqueries.',
        FEEDBACK_SUBQUERY_TYPE:                    'In {{context}}: expected a {{expected}}, got a {{received}}.',
        // grading feedback — task description
    ...
    de: {
        ...
        FEEDBACK_QUERY_EXECUTION_ERROR:   'Die Abfrage ist aufgrund des folgenden Syntaxfehlers nicht ausführbar:',
        FEEDBACK_QUERY_COMPARISON_ERROR:  'Abfragevergleich konnte nicht durchgeführt werden: {{value}}',
        // grading feedback — AST parsing
        ...
        FEEDBACK_SQL_CLAUSE_TYPE:         'Falsche SQL-Klausel, die Aufgabe erfordert eine Klausel vom Typ: {{value}}',
        // grading feedback — columns
        ...
        FEEDBACK_COLUMNS_SOLUTION:        'Die Aufgabe erfordert die Auswahl folgender Spalten: {{value}}',
        // grading feedback — LIMIT / OFFSET
        FEEDBACK_LIMIT_INCORRECT:         'Falscher LIMIT-Wert.',
        FEEDBACK_LIMIT_SOLUTION:          'Erwartet LIMIT {{expected}}, erhalten {{received}}.',
        FEEDBACK_OFFSET_INCORRECT:        'Falscher OFFSET-Wert.',
        FEEDBACK_OFFSET_SOLUTION:         'Erwartet OFFSET {{expected}}, erhalten {{received}}.',
        // grading feedback — JOIN
        FEEDBACK_JOIN_COUNT_WRONG:        'Falsche JOIN-Anweisung: Die Abfrage enthält nicht die richtige Anzahl an JOINs.',
        FEEDBACK_JOIN_WRONG_TABLE:        'Falsche JOIN-Anweisung: Die Abfrage verwendet eine falsche Tabelle in der JOIN-Anweisung.',
        FEEDBACK_JOIN_WRONG_TYPE:         'Falsche JOIN-Anweisung: Die Abfrage verwendet einen falschen JOIN-Typ.',
        FEEDBACK_JOIN_WRONG_CONDITION:    'Falsche JOIN-Anweisung: Die Abfrage verwendet eine falsche JOIN-Bedingung.',
        ...
        // grading feedback — execution plan
        FEEDBACK_PLAN_RETRIEVAL_FAILED:   'Ausführungspläne konnten nicht abgerufen werden.',
        FEEDBACK_PLAN_PARSE_FAILED:       'Ausführungspläne konnten nicht analysiert werden.',
        FEEDBACK_GROUP_KEY:               'Falscher GROUP BY-Schlüssel.',
        FEEDBACK_GROUP_KEY_SOLUTION:      'Erwartet {{expected}}, erhalten {{received}}.',
        ...
        FEEDBACK_DISTINCT_MISSING:        'Das Schlüsselwort DISTINCT fehlt in der Abfrage.',
        FEEDBACK_DISTINCT_MISSING_SOLUTION: 'Die Abfrage sollte SELECT DISTINCT verwenden.',
        FEEDBACK_DISTINCT_EXTRA:          'Das Schlüsselwort DISTINCT sollte in dieser Abfrage nicht verwendet werden.',
        FEEDBACK_DISTINCT_EXTRA_SOLUTION: 'Die Abfrage sollte ein einfaches SELECT ohne DISTINCT verwenden.',
        FEEDBACK_DISTINCT_STRATEGY:       'Hinweis: Unterschiedliche DISTINCT-Implementierungsstrategie (Student: {{student}}, Referenz: {{reference}}).',
        FEEDBACK_CTE_INCORRECT:           'Falsche CTE-Verwendung.',
        FEEDBACK_CTE_SOLUTION:            'Erwartete CTEs: [{{expected}}], erhalten: [{{received}}].',
        FEEDBACK_LIMIT_MISSING:           'LIMIT-Klausel fehlt.',
        FEEDBACK_LIMIT_MISSING_SOLUTION:  'Die Abfrage sollte eine LIMIT-Klausel enthalten.',
        FEEDBACK_LIMIT_EXTRA:             'LIMIT-Klausel sollte in dieser Abfrage nicht vorhanden sein.',
        FEEDBACK_LIMIT_EXTRA_SOLUTION:    'Entfernen Sie die LIMIT-Klausel aus der Abfrage.',
        FEEDBACK_WINDOW_MISSING:          'Fensterfunktion (OVER) fehlt.',
        FEEDBACK_WINDOW_MISSING_SOLUTION: 'Die Abfrage sollte eine Fensterfunktion mit OVER verwenden.',
        FEEDBACK_WINDOW_EXTRA:            'Fensterfunktion (OVER) sollte in dieser Abfrage nicht verwendet werden.',
        FEEDBACK_WINDOW_PARTITION:        'Falsches PARTITION BY in der Fensterfunktion.',
        FEEDBACK_WINDOW_PARTITION_SOLUTION: 'Erwartetes PARTITION BY: [{{expected}}], erhalten: [{{received}}].',
        FEEDBACK_WINDOW_ORDER_BY:         'Falsches ORDER BY in der Fensterfunktion.',
        FEEDBACK_WINDOW_ORDER_BY_SOLUTION: 'Erwartetes Fenster-ORDER BY: [{{expected}}], erhalten: [{{received}}].',
        FEEDBACK_SUBQUERY_COUNT:                   'Falsche Anzahl an Unterabfragen: erwartet {{expected}}, erhalten {{received}}.',
        FEEDBACK_SUBQUERY_COUNT_SOLUTION_SINGULAR: 'Die Abfrage sollte genau {{count}} Unterabfrage enthalten.',
        FEEDBACK_SUBQUERY_COUNT_SOLUTION_PLURAL:   'Die Abfrage sollte genau {{count}} Unterabfragen enthalten.',
        FEEDBACK_SUBQUERY_TYPE:                    'In {{context}}: erwartet ein {{expected}}, erhalten ein {{received}}.',
        // grading feedback — task description
