...
## LLM descriptions via the LLM gateway

LLM-backed description variants (used by `GET /api/generation/generate`, the `llm/*` and `hybrid` endpoints of `/api/description/*`, and the LLM branches of `POST /api/grading/grade`) are generated by calling a generic LLM gateway in the llm-gateway-service `POST /generate` wire format (Vercel AI SDK `UIMessage` format — the gateway's default). The service contains no provider-specific client code and **never reads gateway location or credentials from its own environment** — callers supply them per request:
...
| Field | Required | Description |
|---|---|---|
| `endpoint` | ✓ | Base URL of an llm-gateway-service-compatible API (`/generate` is appended by the service) |
| `apiKey` | ✓ | Sent as `Authorization: Bearer` on every outbound call; never logged |
| `provider` | – | Provider id override. Defaults to `openai`. |
| `model` | – | Model id override. Defaults to `gpt-4o-mini`. |

Temperatures are fixed per strategy: `0` for default/multi-step variants, `0.7` for the creative variant.

### Custom providers (bypass)

To call an OpenAI-compatible endpoint directly — bypassing the gateway's own provider registration — add a `customProvider` override:
...
| Field | Required | Description |
|---|---|---|
| `customProvider.baseUrl` | ✓ | Base URL of an OpenAI-compatible API. The gateway appends `/chat/completions`. |
| `customProvider.apiKey` | ✓ | Provider-specific token for the custom endpoint. Never logged. |

When `customProvider` is present the gateway routes the request directly to `baseUrl` with that key instead of using a registered provider — this is the recommended setup for using arbitrary providers without prior gateway-side registration.

### Template fallback

When a request carries **no usable `llmGateway` block** (missing, incomplete, or invalid), every LLM-dependent description variant falls back to the template engine and response shapes are unchanged:

- `GET /api/generation/generate` still returns the query and all five description fields (the LLM-based variants hold template-generated text).
- The `llm/*` and `hybrid` description endpoints still return a description.
- `POST /api/grading/grade` still produces task-description feedback for non-equivalent queries.

### Breaking change: `OPENAI_API_KEY` removed

The environment-based LLM configuration has been **removed**. The service no longer reads `OPENAI_API_KEY` (or any other variable) to enable, configure, or authenticate LLM work; LLM availability for a request is determined solely by the presence of a usable `llmGateway` block. Deployments that relied on the env-configured key must start sending `llmGateway` per request or accept template-only behavior (which keeps all endpoints functional).
